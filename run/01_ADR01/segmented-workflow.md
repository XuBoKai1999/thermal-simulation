...
| S2 | 0.5–5 s | 0.1 s | candidate, not validated |
...
Solver dt and `--output-every` remain independent. Full dump/VTK output should
be sparse in long segments (for example solver dt 2 s and output every 10 s).
...
```powershell
.\scripts\wsl-run.ps1 "python3 run/01_ADR01/main.py --restart run/01_ADR01/output/dt_0.00025/checkpoint_t_0.05.npz --run-type segment --dt 0.025 --end 0.5 --output-every 0.05"
.\scripts\wsl-run.ps1 "python3 run/01_ADR01/main.py --restart run/01_ADR01/output/dt_0.00025/checkpoint_t_0.05.npz --run-type validation --dt 0.0125 --end 0.5 --output-every 0.05"
...
heat flow `<5%` relative to the finer run's maximum magnitude. Near-zero flows
...
Selection includes each segment start, near-end boundary, maximum absolute sample
slope, maximum absolute sample curvature, points nearest failure thresholds, and
...
Detailed CSV/JSON results, plot, reproducible audit selection, and `report.md`
are under `output/validation_s1/`. No later segment has been accepted.
