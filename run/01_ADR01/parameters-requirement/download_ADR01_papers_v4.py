...
saving an HTML/paywall page as a PDF. On Windows it uses the system certificate store
(via truststore), which is important on managed/corporate networks with HTTPS inspection.
"""

from __future__ import annotations

# On managed Windows networks, HTTPS is often inspected by a corporate proxy.
# Browsers trust the organization's root CA via the Windows certificate store,
# while Python/requests may use a different CA bundle and reject the same sites
# with CERTIFICATE_VERIFY_FAILED.  truststore makes Python use the OS trust store.
try:
    import truststore
    truststore.inject_into_ssl()
except ImportError:
    print("Missing dependency: truststore")
    print("Install dependencies with:")
    print("    python -m pip install requests truststore")
    raise SystemExit(2)
