...
the existing `main.py` remains the owner of one physical segment. Case-local
`restart.py` creates a trusted-checkpoint continuation, while
`validate_continuation.py` post-validates it against a same-checkpoint `dt/2`
reference using `workflow.compare()`.
