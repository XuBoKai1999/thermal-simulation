...
`summary.csv` 只保存區域統計與總熱流等彙總量，不取代逐 cell dump。第一版由 `main.py` 使用 Python 標準庫 `csv` 將 `analyze.py` 回傳的 summary data 寫出，不為此新增另一層 abstraction。ADR01另以FEniCSx native VTK/PVD輸出actual P1 temperature與DG0 heat-flux/region fields；它是interactive FEM visualization path，不取代dump。ADR01 serial runner另以case-local NumPy checkpoint保存exact P1 vector、mesh identity與absolute time，供不同 timestep continuation；這不是generic run-management layer。
